        ...
        for dir_path,dir_names,file_names in os.walk(PATH):
            for fn in file_names:
                if fn[-3:]=='jpg':
                    ...
                    img_raw_data=tf.io.gfile.GFile(image_filename,'rb').read()
                    img_data=tf.image.decode_jpeg(img_raw_data)
                    with tf.compat.v1.Session() as sess:
                        # croped=tf.image.resize_with_crop_or_pad(img_data,250,340)#裁剪
                        resized=tf.image.resize(img_data,[400,400],method=0)#修改像素
                        star=np.array(resized.eval(),dtype='uint8')
                        plt.imshow(star)
                        plt.imsave(image_filename,star)
                        # plt.imshow(croped.eval())
                        # plt.show()

                         # print(im_arr.shape)
                        # x_d=scale(star.reshape(160000).astype('float32'))
                        # x.append(x_d)
    #                     y.append(dir_path)
    #     # print(y)
    #
